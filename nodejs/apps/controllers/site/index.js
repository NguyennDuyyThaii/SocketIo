...
async function cart(req, res) {
    ...
    let productsData = await ProductModel.find({ _id: { $in: productList } })

    // Count sum of product price
    let sum = productsData.reduce((s, i) => s + i.prd_price, 0) // filter, map, reduce => 3 methods đặc trưng xử lý mảng trong JS

    res.render("site/cart", { data: productsData, sum })
}
